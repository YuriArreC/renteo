import Link from "next/link";
import { redirect } from "next/navigation";
import { getTranslations } from "next-intl/server";

import { LegislationPanel } from "@/app/admin/legislation/LegislationPanel";
import { LogoutButton } from "@/components/LogoutButton";
import {
  ApiError,
  type LegislativeAlertListResponse,
  type MeResponse,
} from "@/lib/api";
import { fetchApiServer } from "@/lib/api-server";

export const dynamic = "force-dynamic";

export default async function AdminLegislationPage() {
  let me: MeResponse;
  try {
    me = await fetchApiServer<MeResponse>("/api/me");
  } catch {
    redirect("/login");
  }

  const tCommon = await getTranslations("common");
  const t = await getTranslations("adminLegislation");

  let initial: LegislativeAlertListResponse | null = null;
  let forbidden = false;
  try {
    initial = await fetchApiServer<LegislativeAlertListResponse>(
      "/api/admin/legislative-alerts",
    );
  } catch (err) {
    if (err instanceof ApiError && err.status === 403) {
      forbidden = true;
    } else {
      throw err;
    }
  }

  return (
    <div className="flex min-h-screen flex-col">
      <header className="border-b border-border">
        <div className="container flex h-14 items-center justify-between">
          <div className="flex items-center gap-4">
            <Link
              href="/dashboard"
              className="font-semibold tracking-tight"
            >
              {tCommon("appName")}
            </Link>
            <span className="text-sm text-muted-foreground">
              {me.workspace?.name ?? ""}
            </span>
          </div>
          <LogoutButton />
        </div>
      </header>
      <main className="container flex-1 py-12">
        <h1 className="mb-2 text-3xl font-semibold tracking-tight">
          {t("title")}
        </h1>
        <p className="mb-8 max-w-3xl text-sm text-muted-foreground">
          {t("subtitle")}
        </p>
        {forbidden ? (
          <p className="rounded border border-destructive/40 bg-destructive/5 p-4 text-sm text-destructive">
            {t("forbidden")}
          </p>
        ) : (
          <LegislationPanel
            initialRecords={initial?.records ?? []}
          />
        )}
      </main>
    </div>
  );
}
